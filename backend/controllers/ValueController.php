...
class ValueController extends BehaviorsController
{
    ...
    public function actionCreate($product_id = null)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $data = Value::findOne(['product_id' => $model->product_id, 'attribute_id' => $model->attribute_id]);
            if (!empty($data)) {
                Yii::$app->session->setFlash('error', 'Этот атрибут уже задан');
                return $this->redirect(['/admin/value/create', 'product_id' => $model->product_id]);
            } else {
                $model->save();
