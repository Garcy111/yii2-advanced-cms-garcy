...
class FileUpload {
    ...
    function __construct($uploadName = null) {
        ...
        if ($this->fileName) {
            ...
            $this->newFileName = round(mt_rand(mt_rand(0, 10000), mt_rand(10000, 90000))*time()/1000000) . '.' . $this->fileExtension;
    ...
    private function errorCodeToMsg($code) {
        switch($code) {
            case UPLOAD_ERR_INI_SIZE:
                $message = 'Размер файла привысил лимит.';
                break;
            case UPLOAD_ERR_PARTIAL:
                $message = 'Загруженный файл был загружен только частично.';
                break;
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        if (!$this->fileName) {
            $this->setErrorMsg('Загружаемого файла не существует');
            return false;
        }

        if ($this->fileSize == 0) {
            $this->setErrorMsg('Файл пуст');
            return false;
        }

        if ($this->fileSize > $this->sizeLimit) {
            $this->setErrorMsg('Размер файла привысил лимит');
            return false;
        }
        ...
        if (!file_exists($this->uploadDir)) {
            $this->setErrorMsg('Директория для загружаемых файлов не существует');
            return false;

        } else if (!is_writable($this->uploadDir)) {
            $this->setErrorMsg('Нечитаемый файл');
            return false;
        }
        ...
        if (!$this->save($this->savedFile)) {
            $this->setErrorMsg('Файл не был сохранен');
            return false;
        }
